...
for prompt_len in  512 1024 1536 1920 
    do 
    for gen_len in  128  
    do  
        for bsz in 1 2 5 7 8 10  
        do
            cmd_="--model $model --percent 75 25 0 100 100 0  --gpu-batch-size $bsz \
            ...
            cmd=$cmd_" --overlap false --start_size 4 --recent_size 20"
            outpt="==================================================================================="$'\n'
            ...
            SECONDS=0 
            outpt+=$(CUDA_LAUNCH_BLOCKING=1 numactl --membind=0 --cpubind=0  python -m flexgen.flex_opt $cmd 2>&1 | grep   -e"Total:" -e"Prefill:" -e"Decode:" -e"memory:")
            outpt+="  Execution time: $SECONDS seconds"
            echo "$outpt"  | tee -a opt-13b-results.log
