...
def modified_OPTDecoder_forward(
        ...
        past_key_values_length = self.KVCache_manager.get_past_key_values_length()
...
def modified_opt_attention_forward(
    ...
    k_states = k_states.view(batch_size, q_len, self.num_heads, self.head_dim).transpose(1, 2)
    v_states = v_states.view(batch_size, q_len, self.num_heads, self.head_dim).transpose(1, 2)
    ...
    if k_cache_cpu is not None and cpu_attn_size!=0:
        ...
        attention_mask_q = attention_mask.squeeze(0)[:,:,-q_len:]   if q_len!=1 else None 
    ...
    kv_cache_2add = (k_states, v_states) 
...
def modify_opt_attention(model,KVCache_manager):
    global layer_idx
    config = model.config 
